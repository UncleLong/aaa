    ...
    var B = {
        ...
        GoRender: function (bg, callback) {
            ...
            if (bg && Parameters.RenderSaver[bg]) {
                ...
                Parameters.link = Parameters.RenderSaver[bg].Path.replace("http:", "https:");
            ...
            CallAjax(MomentCam.urls.serverRender, "json=" + JSON.stringify(json), function (result) {
                result.Path = result.Path.replace("square-ali.manboker.com", "square.oss-cn-hangzhou.aliyuncs.com").replace("production-us.manboker.com", "moman-production-us.oss-us-west-1.aliyuncs.com").replace("http:", "https:");
                loadImage(result.Path + Parameters.RenderPrefix, function () {
                    Parameters.clickCheck = true;
                    Parameters.RenderSaver[bg].Path = result.Path.replace("http:", "https:");
                    // console.log("Final Path: " + result.Path);
                    Parameters.link = result.Path.replace("http:", "https:");
...
function CarouseFigureMask(result, update) {
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            Parameters.link = data.Path.replace("http://", "https://");
            data.Path = data.Path.replace("http://", "https://") + Parameters.RenderPrefix + "/resize,w_550";
...
function GoRender(bg, callback) {
    ...
    CallAjax(MomentCam.urls.serverRender, "json=" + JSON.stringify(json), function (result) {
        ...
        result.Path = (result.Path + Parameters.RenderPrefix + "/resize,w_550").replace("http://","https://");
